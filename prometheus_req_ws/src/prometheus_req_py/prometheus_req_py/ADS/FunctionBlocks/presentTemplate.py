...
def managePresent(self,goalHandler) -> tuple[str,int]:
    ...
    if(funcState == reqState.ST_ERROR_CHECK):
        self.get_logger().info(f"[ADS_Node]Checking {goalHandler.request.function_block_name}...")

        self.error_check(f"{goalHandler.request.function_block_name} Error Check",goalHandler)
