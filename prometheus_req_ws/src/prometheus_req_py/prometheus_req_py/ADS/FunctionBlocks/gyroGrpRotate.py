...
def manageGyroGrpRotate(self) -> tuple[str,int]:
    '''
    ...
    funcState=self.plc.read_by_name(f"GVL_ATS.requests.{self.goalHandler.request.function_block_name}.State",pyads.PLCTYPE_INT)
    ...
    if(funcState == reqState.ST_ERROR_CHECK):
        ...
        self.error_check("Gyro Gripper Rotate Error Check")
        self.plc.write_by_name(f"GVL_ATS.requests.{self.goalHandler.request.function_block_name}.errorAck",1,pyads.PLCTYPE_BOOL)
