...
class GUI_node(Node):
    ...
    def start_sequence(self):  
        ...
        if(not self.errorChecked):
            ...
            self.get_logger().info("[GUI_node] Sequence interrupted because error check, resuming sequence")
        ...
        if(not self.functionBlockCalled):
            ...
            self.get_logger().info("[GUI_node] Cannot start sequence, another function block is running.")
    ...
    def call_next_block(self):
        ...
        if self.sequenceAborted:
            self.get_logger().info("[GUI_node] Sequence aborted, stopping...")
            self.sequenceAborted = False
        ...
        try:
            ...
            self.get_logger().info(f"[GUI_node] Calling Block {block} in the sequence")
            self.call_block(block, override=True)
            self.call_next_block()
        except StopIteration:
            self.inSequence=False
            self.functionBlockCalled = False
            self.get_logger().info("[GUI_node] Sequence completed!")
    ...
    def call_block(self, name:str,override=False) -> None:
        ...
        if override or not self.functionBlockCalled:
            ...
            self.get_logger().info(f"[GUI_node] Calling Block {name}")
            ...
            match(name):
                ...
                case "setScrewBayState":
                    ...
                    if(response.result is None):
                        ...
                        self.get_logger().info(f"[GUI_node] Calling setScrewBayState for {self.screwBayReq.bay_number} bays with state: {self.screwBayReq.screw_bays}")
            ...
            self.get_logger().info(f"[GUI_node] Function Block already called, waiting for response...")
    ...
    def goal_feedback_callback(self,feedbackMsg: CallFunctionBlock.Feedback) -> None:
        ...
        match feedbackMsgType:
            case msgType.ERROR_CHECK:
                ...
                if(self.inSequence):
                    self.get_logger().info("[GUI_node] Error check in sequence, aborting sequence!")
            ...
            case msgType.ASK_PICTURE_SCREW:
                ...
                self.get_logger().info(f"[GUI_node] Received picture request with focalPlane:{focalPlane}, roiID:{roiID}, findScrew:{findScrew}")
    ...
    def sendOffsetData(self,msgType,focalPlane:int=0, roiID:int=0, findScrew:bool=False) -> None:
        ...
        self.get_logger().info(f"[GUI_node] Published offset: {offsetMsg}")
    ...
    def state_update_callback(self, msg: EquipmentStatus) -> None:
        ...
        '''
        #Testing code
        #self.get_logger().info("[GUI_node]Receiving:"+str(msg.active_state_fsm_string))
        if(msg is None):
            self.get_logger().error("[GUI_node]Received None msg!")
            return
    ...
    def calculate_picture_offset(self,askedAction,calibrationPlane=0,roiId=0,findScrew=False) -> tuple[bool,float,float,float]:
            ...
            """
            ATS_IP = '10.10.10.100'
            self.get_logger().info(f"[GUI_node]Requesting picture offset from ATS at {ATS_IP}")
            ...
            self.get_logger().info(f"[GUI_node]DATA IS: {response}")
            ...
            if(dataValid is False):
                self.get_logger().error("[GUI_node]Error in picture processing!")
                return False,0.0,0.0,0.0
            #Not used for now
            #circleFound=response["CircleFound"]
            #if(circleFound is False):
            #    self.get_logger().error("[GUI_node]Error in picture processing: Circle not found!")
            #    return False,0.0,0.0,0.0
            translationX=response["TranslationX"]
            translationY=response["TranslationY"]
            rotation=response["Rotation"]

            self.get_logger().info(f"[GUI_node]Picture offset response: {response}")
...
def main(args=None):
    ...
    def update_ros():
        ...
        '''
        rclpy.spin_once(gui_node,timeout_sec=0.005)
        #gui_node.get_logger().info("[GUI_node]Spinning once...")
    ...
    def on_close():
        ...
        gui_node.get_logger().info("[GUI_node] Shutting down...")
